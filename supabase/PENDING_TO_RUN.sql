...
-- ========== HOME / GALLERY / ROOMS PAGE STORAGE ==========
...
notify pgrst, 'reload schema';
-- Gallery page photos (run in Supabase SQL Editor)
-- Public bucket for admin gallery uploads. Safe to re-run.
...
drop policy if exists "Public can view gallery page images" on storage.objects;
drop policy if exists "Admins can upload gallery page images" on storage.objects;
drop policy if exists "Admins can update gallery page images" on storage.objects;
drop policy if exists "Admins can delete gallery page images" on storage.objects;

create policy "Public can view gallery page images"
  on storage.objects
  for select
  to anon, authenticated
  using (bucket_id = 'gallery-page');

create policy "Admins can upload gallery page images"
  on storage.objects
  for insert
  to authenticated
  with check (bucket_id = 'gallery-page' and public.is_approved_admin());

create policy "Admins can update gallery page images"
  on storage.objects
  for update
  to authenticated
  using (bucket_id = 'gallery-page' and public.is_approved_admin())
  with check (bucket_id = 'gallery-page' and public.is_approved_admin());

create policy "Admins can delete gallery page images"
  on storage.objects
  for delete
  to authenticated
  using (bucket_id = 'gallery-page' and public.is_approved_admin());
